...
fn main() {
    let cfg = parse_args().unwrap_or_else(|msg| {
        eprintln!("{msg}");
        eprintln!("Usage: cargo run --bin defaulthash -- [--ops N] [--keys K] [--mode both|put|get]");
        eprintln!("Defaults: --ops 5000000 --keys 20000");
        std::process::exit(2);
    });

    let keys = generate_keys(cfg.keys);
    let (put, get) = run_bench(&keys, cfg.ops, cfg.mode);

    println!("Default HashMap benchmark (6-byte keys)");
    println!("keys: {}, ops: {}, mode: {}", cfg.keys, cfg.ops, mode_name(cfg.mode));
    if let Some((total, ns_per_op)) = put {
        println!(
            "put: total = {:.3} ms, ns/op = {:.2}",
            total.as_secs_f64() * 1_000.0,
            ns_per_op
        );
    }
    if let Some((total, ns_per_op)) = get {
        println!(
            "get: total = {:.3} ms, ns/op = {:.2}",
            total.as_secs_f64() * 1_000.0,
            ns_per_op
        );
    }
...
fn run_bench(keys: &[Mac], ops: usize, mode: BenchMode) -> (Option<(Duration, f64)>, Option<(Duration, f64)>) {
    let put = if mode == BenchMode::Both || mode == BenchMode::Put {
        Some(run_put(keys, ops))
    } else {
        None
    };
    let get = if mode == BenchMode::Both || mode == BenchMode::Get {
        Some(run_get(keys, ops))
    } else {
        None
    };
    (put, get)
...
fn run_put(keys: &[Mac], ops: usize) -> (Duration, f64) {
    let mut map: HashMap<Mac, u64, StdRandomState> =
        HashMap::with_capacity_and_hasher(keys.len() * 2, StdRandomState::new());
    ...
    black_box(map.len());
    let put_ns_per_op = (put_total.as_nanos() as f64) / (ops as f64);
    (put_total, put_ns_per_op)
}

fn run_get(keys: &[Mac], ops: usize) -> (Duration, f64) {
    let mut map: HashMap<Mac, u64, StdRandomState> =
        HashMap::with_capacity_and_hasher(keys.len() * 2, StdRandomState::new());
    ...
    black_box(checksum);
    let get_ns_per_op = (get_total.as_nanos() as f64) / (ops as f64);
    (get_total, get_ns_per_op)
}
